   ...
   case WM_COMMAND :
      ...
      case 1002:
         ...
         if( strlen(cmdhrinstall) != 0 )		      
            {
            msgOK("Attention! CmdHere seems to already be installed!", "CmdHere must be uninstalled before it can be installed again.");
            return(0);
            }		          
         ...
         if (tlen < 2)
            {
            msgOK("Attention! Invalid path.", "You must enter a valid path to install CmdHere!.");
            return(0);
            }  
         ...
         if( stricmp(installDIR, cPath) == 0 )
            ...
            if( cPath[strlen(cPath)-1] != '\\' )
               strcat(cPath, "\\");
            strcat(cPath, "CmdHere.dll");   

            origobj = WinQueryObject(cPath);
            if( origobj == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK("Error!", "Could not find CmdHere.dll. Installation aborted.");
            ...
            if( WinCopyObject(origobj, destobj, 0) == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK(installDIR, "Could not copy CmdHere.dll to destination path (File may already exist). Installation aborted.");
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
               return(0);
            ...
            if( cPath[strlen(cPath)-1] != '\\' )
               strcat(cPath, "\\");
            strcat(cPath, "cmdhres.dll");   
            
            origobj = WinQueryObject(cPath);
            if( origobj == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK("Error!", "Could not find cmdhres.dll. Installation aborted.");
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
               return(0);
            ...
            if( WinCopyObject(origobj, destobj, 0) == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK(installDIR, "Could not copy cmdhres.dll to destination path (File may already exist). Installation aborted.");
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
               return(0);
            ...
            if( cPath[strlen(cPath)-1] != '\\' )
               strcat(cPath, "\\");
            strcat(cPath, "cmdhinst.exe");   
            
            origobj = WinQueryObject(cPath);
            if( origobj == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK("Error!", "Could not find cmdhinst.exe. Installation aborted.");
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
               return(0);
            ...
            if( WinCopyObject(origobj, destobj, 0) == NULLHANDLE )
               {
               DosBeep(700, 100);
               msgOK(installDIR, "Could not copy cmdhinst.exe to destination path (File may already exist). Installation aborted.");
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
               return(0);
         ...
         if( rc == 0 )
            ...
            if( rc == 0 )
               ...
               WinRegisterObjectClass("cmdherefolder", cmdDLL);
               WinReplaceObjectClass("WPFolder", "cmdherefolder", TRUE);
               WinRegisterObjectClass("cmdhroot", cmdDLL);
               WinReplaceObjectClass("WPDisk", "cmdhroot", TRUE);
               ...
               if( UPDATECONFIG )
                  {		    
                  msgOK("Attention!", "CmdHere has been registered and installed! Reboot for it to be activated");
                  }
               else
                  {   
                  msgOK("Attention!", "CmdHere has been registered! Update your LIBPATH statement in config.sys and Reboot for it to be activated");
                  }
               WinEnableWindow(WinWindowFromID(hwndDlg, 1000), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1001), TRUE);
               WinEnableWindow(WinWindowFromID(hwndDlg, 1002), TRUE);
